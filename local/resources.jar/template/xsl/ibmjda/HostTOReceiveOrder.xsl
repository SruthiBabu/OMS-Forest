			...
			<xsl:for-each select="//receivingAdvice" >
				<xsl:element name="API" >
					<xsl:attribute name="Name">
						<xsl:text>receiveOrder</xsl:text>
					</xsl:attribute>
					<xsl:element name="Input" >		
							...
							<xsl:variable name="ExtEnterprise">
							...
							<xsl:element name="Receipt">								
								...
								<xsl:attribute name="DocumentType">
									...
									<xsl:attribute name="EnterpriseCode">
										<xsl:value-of select="ValueMaps:getMapValue($ibmjda:ValueMapsData, '', 'jdaToIBMOrganizationCode', $ExtEnterprise)"/> 
									</xsl:attribute>
								</xsl:element>	
