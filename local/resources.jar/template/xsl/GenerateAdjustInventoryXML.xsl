<?xml version='1.0'?>
<!-- Stylesheet for transforming EXTERNAL_SUPPLY_CHANGE xml to adjustInventory input xml-->
...
<xsl:template match="/">		
	...
	<xsl:attribute name="QuickMode">N</xsl:attribute>
		...
		<xsl:for-each select="/Items/Item/SupplyChanges/Supply">
			<xsl:variable name="sIsInboundOrder">
					<xsl:value-of select="@IsInboundOrder"/>
			</xsl:variable>						
			<xsl:choose>						
				<xsl:when test="$sIsInboundOrder != 'N'">
				...
				<xsl:attribute name="TranCode">PRMS</xsl:attribute>
				<xsl:attribute name="ETA"><xsl:value-of select="@ETA"/></xsl:attribute>
