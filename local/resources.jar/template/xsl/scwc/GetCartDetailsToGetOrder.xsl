...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="OrderToProcessOrder" match="/">
		...
		<xsl:variable name="cartId">
			<xsl:choose>
				<xsl:when test="starts-with(/Order/@CartId, 'WC_')">
					<xsl:value-of
						select="substring-after(/Order/@CartId, 'WC_')" />
				</xsl:when>
				<xsl:otherwise>
			...
			<_ord:DataArea>
				<oa:Get>
					<oa:Expression expressionLanguage="_wcf:XPath">
						{_wcf.ap=IBM_Details}/Order[OrderIdentifier[(UniqueID='<xsl:value-of select="$cartId"/>')]]
					</oa:Expression>
				</oa:Get>
			</_ord:DataArea>
		</_ord:GetOrder>
