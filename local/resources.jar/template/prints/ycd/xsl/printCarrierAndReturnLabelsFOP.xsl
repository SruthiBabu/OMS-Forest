...
<xsl:template match="/">
		<fo:root>
			...
			<fo:flow flow-name="xsl-region-body"
					font-family="Arial">
					<xsl:apply-templates
						select="getTrackingNoAndPrintLabel/SuccessfulContainers/Container" />
							 				
					<xsl:apply-templates
						select="getTrackingNoAndPrintLabel/SuccessfulContainers/Container/ContainerReturnTrackingList" />
				</fo:flow>
			</fo:page-sequence>
		</fo:root>
</xsl:template>
			...
			<fo:table-body>
					<fo:table-row>
						...
						<fo:block font-family="Arial"
							...
							space-before="100mm" text-align-last="left">
	...
	<xsl:template
		match="getTrackingNoAndPrintLabel/SuccessfulContainers/Container/ContainerReturnTrackingList">
		<fo:table background-color="White" table-layout="fixed"
			border-width="0mm" width="100%">
			<fo:table-column column-width="100%" />
			<fo:table-body>
				<xsl:for-each select="ContainerReturnTracking">
					<fo:table-row>
						<fo:table-cell>
							<fo:block font-family="Arial"
								...
								<xsl:variable name="path"
									select="concat(/getTrackingNoAndPrintLabel/SuccessfulContainers/Container/@PierbridgeServerURL, '?', @PierbridgeReturnLabelURL)">
								</xsl:variable>
								<fo:external-graphic height="6in" width="4in" content-height="scale-to-fit" content-width="scale-to-fit"  src="url({$path})" />
							</fo:block>
