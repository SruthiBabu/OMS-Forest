...
<xsl:stylesheet version="1.0"
	...
	<xsl:apply-templates select="ContainerReturnTrackingList" />
	</xsl:template>
	
	<xsl:template match="ContainerReturnTrackingList">
						...
						<fo:table-cell>
							<fo:block font-family="Arial"
								...
								<xsl:variable name="path"
									select="concat(/reprintCarrierLabel/Container/@PierbridgeServerURL, '?', @PierbridgeReturnLabelURL)">
						</xsl:variable>
								<fo:external-graphic
									height="6in" width="4in" content-height="scale-to-fit" content-width="scale-to-fit"
									src="url({$path})" />
							</fo:block>
						</fo:table-cell>
					</fo:table-row>
